                ...
                {categoreis.map((category, index)=>(
                    <button key={index} onClick={() => SetActiveCategory(category)} 
                        className={cn('px-5 py-2 rounded-full transition-colors duration-300 capitalize',
                            activeCategory === category ? 'bg-primary text-primary-foreground' : 'bg-secondary/70 text-forefound hover:bg-secondary'
                    )}> {category} </button>
                ))}            
            </div>
            ...
            <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6'>
                {filteredSkills.map((skill, index) => (
                    ...
                    <div key={index} className='bg-card p-6 rounded-lg shadow-xs card-hover'>
                        ...
                        <div className='bg-primary h-2 rounded-full origin-left animate-[glow_1.5_ease-out]' style={{ width: skill.level+ "%"}} >
                            <CountUp />
                        </div>
                        </div>
                        ...
                        <div className='text-right mt-1'>
                            <span className='text-sm font-semibold text-muted-foreground'> <CountUp end={skill.level} duration={10} />% </span>
                        </div>
